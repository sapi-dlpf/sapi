...
def inicializar():
    ...
    try:
        ...
        if (Gconfiguracao is not None):
            ...
            erro="Programa não pode ser executado, pois não existe configuração para este agente. Configure-o no SETEC3 (SAPI -> Configuração -> Configuração de Agentes)."
            print_log(erro)
        ...
        # Algo inesperado
        erro="Houve alguma falha exótica sem solução conhecida. Verifique o log na pasta sistema/log para mais detalhes. Vamos encerrar e aguardar atualização, pois pode ser algum defeito na interface entre cliente e servidor. Erro detectado: " + str(e)
        print_log(erro)
        sapisrv_reportar_erro(erro)
        # Ao retornar False, o chamador entenderá que tem que atualizar o sapi_iped, e encerrará a execução
        return False
...
def executar_nova_tarefa():
    ...
    if Gconfiguracao["storage_unico"] != "":
        print_log("Este agente trabalha apenas com storage:", Gconfiguracao["storage_unico"])
        tarefa = solicita_tarefas(Glista_ipeds_suportados, Gconfiguracao["codigo_unidade_organizacional"], Gconfiguracao["storage_unico"])
        outros_storages = False
    elif Gconfiguracao["storage_preferencial"] != "":
        print_log("Este agente trabalha com storage preferencial:", Gconfiguracao["storage_preferencial"])
        tarefa = solicita_tarefas(Glista_ipeds_suportados, Gconfiguracao["codigo_unidade_organizacional"], Gconfiguracao["storage_preferencial"])
        outros_storages = True
    else:
        print_log("Este agente trabalha com QUALQUER storage")
        outros_storages = True
