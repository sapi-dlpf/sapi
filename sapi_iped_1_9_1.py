...
def inicializa_dados_laudo():
    global Gdados_laudo

    Gdados_laudo = dict()

# Armazena em dados para utilização no laudo
def armazena_dados_laudo(label, valor):
    global Gdados_laudo
...
def inicializar():
    ...
    global Glista_ipeds_suportados
    ...
    try:
        ...
        if (Gconfiguracao is not None):
            ...
            erro="Programa não pode ser executado, pois não existe configuração para este agente. Para criar a configuração, rode sapi_iped.py novamente com a opção --config"
            print_log(erro)
        ...
        # Algo inesperado
        erro="Houve alguma falha incomum sem tratamento definido. Verifique o log na pasta sistema/log para mais detalhes. Erro detectado: " + str(e)
        print_log(erro)
        sapisrv_reportar_erro(erro)
        # Ao retornar False, o chamador entenderá que tem que atualizar o sapi_iped, e encerrará a execução
        return False
    ...
    Glista_ipeds_suportados=Gconfiguracao["lista_tipo_tarefa"]

    if (Glista_ipeds_suportados)==0:
...
def solicita_tarefas(lista_tipos, unidade, storage=None):
    for tipo in lista_tipos:

        # Registra em log
        log = "Solicitando tarefa com tipo=[" + tipo + "], unidade=[" + str(unidade) + "]"
        ...
        # Requisita tarefa
        (disponivel, tarefa) = sapisrv_obter_iniciar_tarefa(tipo, unidade=unidade, storage=storage)
        if disponivel:
...
def solicita_tarefa_exclusao(lista_tipos, codigo_unidade_organizacional, storage=None):
    for tipo in lista_tipos:
        ...
        # Requisita tarefa
        (disponivel, tarefa) = sapisrv_obter_excluir_tarefa(tipo, codigo_unidade_organizacional, storage=storage)
        if disponivel:
...
def excluir_tarefa():
    ...
    tarefa = None
    unidade = Gconfiguracao["codigo_unidade_organizacional"]
    ...
    if tarefa is None and outros_storages:
        ...
        tarefa = solicita_tarefa_exclusao(Glista_ipeds_suportados, unidade)
...
def montar_comando_iped(iped_caminho_jar,
                        caminho_origem,
                        caminho_destino,
                        profile_iped):
    ...
    # ========================================================
    comando = "java -jar " + iped_caminho_jar

    # Adiciona o perfil do IPED
    comando = comando + " -profile " + profile_iped
...
def executar_nova_tarefa():
    ...
    if Gconfiguracao["storage_escopo"] == CONST_storage_unico():
        print_log("Este agente trabalha apenas com storage:", Gconfiguracao["storage_selecionado"])
        tarefa = solicita_tarefas(Glista_ipeds_suportados, Gconfiguracao["codigo_unidade_organizacional"], Gconfiguracao["storage_selecionado"])
        outros_storages = False
    elif Gconfiguracao["storage_escopo"] == CONST_storage_preferencial():
        print_log("Este agente trabalha com storage preferencial:", Gconfiguracao["storage_selecionado"])
        tarefa = solicita_tarefas(Glista_ipeds_suportados, Gconfiguracao["codigo_unidade_organizacional"], Gconfiguracao["storage_selecionado"])
    ...
    if tarefa is None and outros_storages:
        # Solicita tarefa para qualquer storage
        tarefa = solicita_tarefas(Glista_ipeds_suportados, Gconfiguracao["codigo_unidade_organizacional"])
    ...
    armazena_dados_laudo('sapiIpedProfile', profile_iped)
    Gdados_laudo['sapiIpedProfile'] = profile_iped

    (comando, caminho_tela_iped, caminho_log_iped)=montar_comando_iped(
                        Gconfiguracao["iped_caminho_jar"],
                        caminho_origem,
                        caminho_destino,
                        profile_iped)
...
# ======================================================================
# Funções relacionadas com a Configuração do sapi_iped
# ======================================================================

# Carrega configuração de arquivo
# ----------------------------------------------------------------------
# Deprecated: A configuração SEMPRE VEM DO SERVIDOR
# def cfg_carregar():
#     # Irá atualizar as duas variáveis relacionadas com estado
#     global cfg
# 
#     # Não tem arquivo de estado
#     if (not os.path.isfile(Garquivo_cfg)):
#         return False
# 
#     # Le dados do arquivo e fecha
#     arq = open(Garquivo_cfg, "r")
#     cfg = json.load(arq)
#     arq.close()
# 
#     # Tudo certo
...
def _cfg_definir():
    ...
    # Dica para wiki
    wiki_sapi="http://10.41.84.5/wiki/index.php/SAPI_Manual_de_instalação_e_configuração"
    ...
    # ----------------------------------
    configuracao_existente = sapisrv_obter_configuracao_cliente(Gprograma)
    if configuracao_existente!=None:
        print_atencao()
        print("- Este agente já possui configuração definida no servidor.")
        ...
        print("4) O IPED deve ter sido instalado adequadamente nesta máquina.")
        print("   Dica: Não esqueça de revisar o LocalConfig.txt e revisar/criar profiles customizados")
        print("- Em caso de dúvida sobre algum destes pontos, ver: ", wiki_sapi)
        print()
        if not prosseguir_configuracao():
            return False
    ...
    # --------------------------------------
    print()
    print("Login no SAPI/SETEC3")
    print("--------------------")
    print("Na janela gráfica que foi aberta, forneça sua credencial de acesso ao SETEC3, que é igual à credencial do Siscrim")
    usuario=autenticar_usuario()
    if (usuario is None):
        print_tela("- Não foi fornecida credencial válida. Comando cancelado")
        return False

    # Ok, autenticação efetuada
    #var_dump(usuario)
    #die('ponto2152')

    nome_guerra=usuario['nome_guerra']
    sigla_uni=usuario['sigla_uni']
    unidade=usuario['codigo_unidade_lotacao']
    ip_cliente = usuario["ip_cliente"]
    host_name = socket.gethostname()

    cfg['configurador_conta_usuario'] = usuario['conta_usuario']
    cfg['configurador_unidade'] = unidade
    cfg['configurador_unidade_sigla'] = sigla_uni
    cfg['computador_ip'] = ip_cliente
    cfg['computador_hostname'] = host_name

    #print("- Autenticado com sucesso:",nome_guerra, "-", sigla_uni)
    print("- Usuário: ",nome_guerra)
    print("- Unidade de lotação:", sigla_uni)
    print("- Máquina:", ip_cliente, host_name)

    # Verifica se usuário tem direito de administrador
    if not usuario["adm_sapi"]:
        print("- ERRO: Para efetuar a configuração você precisa de direito de administrador no SAPI.")
        print("  Ver:",wiki_sapi)
        return False

    if usuario['fora_da_unidade_lotacao']:
        print_tela("- Operação reservada para administrado SAPI. Você não dispõe de direito de administrador do SAPI estando fora da sua unidade de lotação. Ver: ",wiki_sapi)
        return False
    ...
    # -----------------------------------------------------------
    print()
    print("Storages habilitados na unidade")
    print("-" * 65)
    #print_tela_log("- Recuperando lista de storages. Aguarde...")
    lista_storages=obter_lista_storages_unidade(unidade)
    if not isinstance(lista_storages, list):
        # Algum erro aconteceu...finaliza
        print("- Lista de storages inconsistente")
        return False

    # Se ainda não tem nenhum storage, finaliza
    if len(lista_storages)==0:
        print("- ERRO: Para configurar o sapi_iped deve existir pelo menos um storage registrado na unidade",sigla_uni)
        print("  Ver:",wiki_sapi)
        return False
    ...
    for sto in lista_storages:
        ...
        if (q == 1):
            ...
            print("-"*65)
        # Exibe elemento
        print('%2d  %-60s' % (q, sto["storage_id"] + " : " + sto["nome_storage"]))


    # Recupera lista de perfis de IPED da unidade de lotação do usuário
    # -------------------------------------------------------------------
    print()
    print("Lista profiles IPED habilitados no SAPI")
    print("-" * 100)
    dict_perfil=obter_dict_perfil_iped(unidade)
    #var_dump(dict_perfil)
    ...
    if len(dict_perfil)==0:
        print("- ERRO: Não existe nenhum perfil para sapi_iped configurado na unidade",sigla_uni)
        print("  Ver:",wiki_sapi)
        return False
    ...
    for id_perfil in dict_perfil:
        ...
        if (q == 1):
            # Cabecalho
            print('%2s %-15s %-90s' % ("", "Profile", "Descrição"))
            print("-"*100)
        # Exibe elemento
        print('%2d %-15s %-90s' % (q, id_perfil, dict_perfil[id_perfil]))

    print()
    print("- Verifique se as listas de storage e de profiles acima correspondem à sua expectativa.")
    print("- Caso contrário, para evitar retrabalho, primeiro ajuste a configuração destes elementos e depois retorne aqui.")
    if not pergunta_sim_nao("< Prosseguir?", default="n"):
        print("- Configuração cancelada pelo usuário.")
        return False

    cfg["iped_profiles_testados"]=list(dict_perfil.keys())


    # Escopo de storages
    # -----------------------------------------------
    lista_opcoes=[
        (CONST_storage_unico(), "Processa tarefas de um único storage."),
        (CONST_storage_preferencial(), "Processa tarefas de um certo storage (preferencial), "
                          "\n    mas em caso de ociosidade processa tarefas de qualquer storage"),
    ...
    if selecionar_storage:
        ...
        storage_teste=storage
    else:
        # Se não tem um storage específico associado,
        # pega o primeiro para fazer teste de conexão
        storage_teste=lista_storages[0]
        print("- Como este sapi_iped dever funcionar com qualquer storage, será utilizado no teste o", storage_teste["nome_storage"])
    ...
    print("-------------------")
    ponto_montagem = conectar_storage_atualizacao_ok(storage_teste)
    if ponto_montagem is None:
        # Mensagens de erro já foram apresentadas pela função acima
        return False
    print("- Conectado com sucesso no storage", storage_teste["nome_storage"])
    print("- Pasta de montagem:", ponto_montagem)
    ...
    if not ('java version' in saida):
        print("- ERRO: Não foi encontrado indicativo de versão java")
        return False
    if not ('64-Bit' in saida):
        print_atencao()
        print("- AVISO: Versão java não parece ser 64 bits. Confira")
        if not pergunta_sim_nao("< Prosseguir?", default="n"):
            print("- Configuração cancelada pelo usuário.")
            return False
    ...
    # Comando para execução do IPED
    comando='java -jar ' + iped_caminho_jar + ' --help '

    # Saida para arquivo
    caminho_tela_iped = "teste_iped_tela_saida.txt"
    comando = comando + " >" + caminho_tela_iped + " 2>&1 "

    print("- Invocando iped com comando")
    print(comando)
    (sucesso, msg_erro, saida) = executa_comando_os(comando)
    # Aqui não podemos testar se teve sucesso,
    ...
    if versao_iped=="":
        ...
        print()
        print_tela_log("- Não foi possível reconhecer a versão do IPED. Assegure-se que IPED e seus requisitos estão adequadamente instalados")
        return False
    ...
    print_tela_log("- IPED invocado com sucesso.")
    ...
    if versao_iped not in lista_versoes_iped:
        print_atencao()
        print_tela_log("- Versão do IPED", versao_iped, "detectada não está na lista de versões homologadas pelo sapi_iped",str(lista_versoes_iped))
        print("- Isto não significa que não irá funcionar. Significa apenas que esta versão nunca foi testada com o sapi_iped.")
        print("- Versões com formatos de saída e log muito diferentes das homologadas, podem apresentar mal funcionamento, ")
    ...
    print("----------------------------------")
    print("- A seguir serão efetuados vários testes para assegurar que o IPED está adequadamente configurado para ser utilizado pelo SAPI.")
    ...
    if not prosseguir_configuracao():
        return False
    if not _cfg_testar_iped(storage_teste, dict_perfil, cfg['iped_caminho_jar']):
    ...
    print("-"*100)
    cfg_utf8 = console_sanitiza_utf8_ok(cfg)
    ...
    print("- Se você prosseguir, a configuração será registrada no SETEC3 e o sapi_iped desta máquina passará a utilizá-la.")
    if not prosseguir_configuracao():
        return False
...
def _cfg_testar_iped(storage_teste, dict_perfil, iped_caminho_jar):
    ...
    for profile_iped in dict_perfil:
        ...
        while True:
            ...
            (sucesso, mensagem_erro, conteudo_tela, conteudo_log) = \
                testar_iped(storage_teste, iped_caminho_jar, profile_iped)
            ...
            if sucesso == False:
                print("- Tentativa de execução do IPED falhou.")
                ...
                # analisar_erro_iped()
                print()
                print("- Analise o erro acima, e corrija a configuração do IPED, e quando estiver tudo ok, tente novamente")
...
def testar_iped(storage_teste, iped_caminho_jar, profile_iped):
    ...
    print("- Dados para teste serão gravados na pasta:", pasta_teste_memorando)
    # Se já existe pasta de teste, exclui antes de prosseguir
    if os.path.exists(pasta_teste_memorando):
        print("- Pasta já existe. Excluindo pasta.")
        try:
            shutil.rmtree(pasta_teste_memorando)
        except Exception as e:
            print("- ERRO: Não foi possível limpar pasta de teste: " + str(e))
            return False

    # Não pode existir. Se existir, processo de exclusão acima falhou
    if os.path.exists(pasta_teste_memorando):
        print("- [2481] ERRO: Tentativa de excluir pasta de teste falhou: Verifique se existe algo aberto na pasta, que esteja criando um lock em algum dos seus recursos")
        return False
    try:
        os.makedirs(pasta_teste_memorando)
    except Exception as e:
        print("- [2488] ERRO: Não foi possível criar pasta de teste: " + str(e))
        return False

    # Confere se deu certo
    if not os.path.exists(pasta_teste_memorando):
        print("- [2493] ERRO: Criação de pasta de teste falhou sem causar exceção!! Situação inesperada!")
        return False
    ...
    try:
        ...
        print("- Criando arquivo: ", caminho_arquivo_teste)
        file = open(caminho_arquivo_teste, "w")
        file.write("Arquivo para testar execução do IPED")
        file.close()

    except Exception as e:
        print("- [2488] ERRO: Não foi possível criar arquivo para teste do iped: " + str(e))
        return False
    ...
    print("- Criando pasta para destino do IPED: ", caminho_destino)
    try:
        os.makedirs(caminho_destino)
    except Exception as e:
        print("- [2488] ERRO: Não foi possível criar pasta:  " + str(e))
        return False
    # Confere se deu certo
    if not os.path.exists(caminho_destino):
        print("- [2493] ERRO: Criação de pasta falhou sem causar exceção!! Situação inesperada!")
        return False
    ...
    #     erro_exception = str(e)
    #     deu_erro = True
    print("- Executando IPED com comando abaixo. Pode demorar alguns minutos. Aguarde...")
    print(comando)
    (sucesso, msg_erro, saida) = executa_comando_os(comando)
