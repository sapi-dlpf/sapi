...
def obter_quesitos_padroes():
    ...
    while True:
        ...
        print("- Recuperando quesitos repetidos. Aguarde (ISTO PODE DEMORAR ALGUNS MINUTOS)...")
...
def _adicionar_quesitos():
    ...
    # -----------------------------------
    print()
    print("Passo 1: Informe o arquivo que contém o modelo de laudo da sua unidade")
    print("----------------------------------------------------------------------")
    print("- Informe o arquivo de que contém o modelo de laudo da sua unidade.")
    ...
    # kkkk
    print("Passo 2: Quesitos padrões")
    print("-------------------------")
    ...
    if ajustar_modelo_unidade(caminho_arquivo_saida_odt, quesitacoes, parametros):
        print()
        print("- Não esqueça de carregar no SisCrim o modelo criado:", caminho_arquivo_saida_odt)
        print_atencao()
        print("- Na mesma tela de inclusão de modelo existem duas tabelas distintas: modelos de usuário e modelos da unidade")
        print("  Para que todos na sua unidade tenha acesso ao modelo SAPI, ")
        print("  ele deve ser cadastrado como um MODELO DA UNIDADE (não como um modelo de Usuário)")
        print()
...
def criar_modelo_unidade(caminho_arquivo_entrada_odt, nome_arquivo_saida):
    ...
    if os.path.isfile(caminho_arquivo_saida_odt):
        print_atencao()
        print("- Arquivo de modelo da unidade já existe:", caminho_arquivo_saida_odt)
...
#
def ajustar_modelo_unidade(caminho_arquivo_saida_odt, quesitacoes, parametros):
    ...
    # Isola linha de modelo da tabela de materiais
    tr_modelo_quesito = tabela_quesitacoes.findall('table:table-row', Gxml_ns)[0]

    #print()
    #odt_dump(tr_modelo)

    tab_quesitacoes = Godt_filho_para_pai_map[tr_modelo_quesito]
    posicao_inserir_tab_quesitacoes = odt_determina_posicao_pai_filho(tab_quesitacoes, tr_modelo_quesito)
    ...
    for quesitacao in quesitacoes:
        ...
        print_tela_log("- Processando quesitação ", quesitacao['hash'])
    ...
    if not odt_substitui_content_xml(Godt_raiz, caminho_arquivo_saida_odt):
        print("- Comando cancelado")
        return False
    ...
    print_tela_log("- Caminho: ", caminho_arquivo_saida_odt)
    print()
    if pergunta_sim_nao("< Deseja abrir o modelo para edição (será invocado o Libreoffice)?"):
        webbrowser.open(caminho_arquivo_saida_odt)
    ...
    print("- A seguir será dado opção para você entrar em modo de validação")
    print("- Neste modo, você pode ir alterando o modelo, e fazendo validações logo em seguida")
    print()
    if pergunta_sim_nao("< Deseja entrar em modo de validação do modelo (*VM)?"):
        loop_validacao_modelo(caminho_arquivo_saida_odt)

    # Ok, deu tudo certo, retorna o caminho do arquivo criado
    return True
...
def obter_lista_respostas_padroes(texto_quesito):
    ...
    resposta['sapiRespostaIdentificarUsuario'] = ['julgados úteis', 'úteis']
