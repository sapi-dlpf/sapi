...
Gmenu_comandos['comandos'] = {
    ...
    '*gl': 'Gera laudo',
    '*vl': 'Validar modelo de laudo (utilizado por Administrador SAPI)',
...
Gmenu_comandos['cmd_geral'] = ['*ml', '*gl', '*vl', '*s3', '*s3g', '*sto', '*cl',  '*tt', '*qq']
...
def odt_recupera_lista_campos_variaveis_sapi(pai):
    ...
    for cv in lista_cv:
        ...
        nome = cv["name"].lower()
        if (nome[:4] != "sapi"):
            print_tela_log("- ERRO: Campo [", cv["name"], "] com nome fora do padrão (Formato correto: sapiXXXX")
            erros += 1
            continue
...
def ajustar_laudo_odt(caminho_arquivo_entrada_odt):
    ...
    if (Gxml_ns.get("text", None) is None):
        ...
        print_tela_log(
            "- Se você está regerando o laudo, assegure-se de ter escolhido o arquivo SEM sufixo _sapi (ou seja, o arquivo orginal, gerado pelo SisCrim).")
        return
    ...
    modelo_versao="MODELO_VERSAO_2_3"
    if (modelo_versao not in Gmod_constantes):
        print_tela_log(
...
# ==================================================================================================================
# @*vl - validar modelo SAPI
# Utilizado para determinar se o modelo de laudo contém erros
# ==================================================================================================================

# Validar laudo
# ----------------------------------------------------------------------------------------------------------------------
def validar_modelo_laudo_sapi():
    console_executar_tratar_ctrc(funcao=_validar_modelo_laudo_sapi)


def _validar_modelo_laudo_sapi():
    ...
    print("- Isto elmina a chance de se fazer o upload de um modelo com algum problema série que inviabilize o seu uso")
    ...
    print()
    print("Passo 1: Selecione arquivo de modelo de laudo SAPI")
    print("--------------------------------------------------")
    ...
    # Tudo certo
    _executa_validacao_modelo(caminho_modelo)


def _executa_validacao_modelo(caminho_modelo):
    ...
    # O arquivo xxxx.odt será copiado para xxxx_sapi.odt
    # Caminho para o novo arquivo gerado
    (pasta_entrada, nome_arquivo_entrada) = decompoe_caminho(caminho_modelo)
    ...
    # Copia arquivo de entrada para arquivo de saída
    pasta_saida = pasta_entrada
    nome_arquivo_saida = nome_arquivo_entrada.replace(".odt", "_validacao.odt")
    caminho_arquivo_saida_odt = pasta_saida + nome_arquivo_saida
    print_tela_log("- Copiando para arquivo de saída: " + caminho_arquivo_saida_odt)
    try:
        # Tenta duplicar arquivo, para equivalente com extensão SAPI
        shutil.copyfile(caminho_modelo, caminho_arquivo_saida_odt)
    except BaseException as e:
        print_tela_log("- ERRO na criação do arquivo de saída '" + caminho_arquivo_saida_odt + "'")
        print_tela_log("- ERRO: ", e)
        print_tela_log(
    ...
    valida_modelo(caminho_arquivo_saida_odt, validando_modelo=True)


def valida_modelo(caminho_arquivo_saida_odt, validando_modelo=False):
