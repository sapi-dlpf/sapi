...
# Variáveis atualizadas  durante validação do modelo
Gdblocos                = None
...
def ajustar_laudo_odt(caminho_arquivo_entrada_odt):
    ...
    if not carrega_valida_modelo(caminho_arquivo_saida_odt, sapi_admim=False):
    ...
    gera_novo_odt(Godt_raiz, caminho_arquivo_saida_odt)
...
def _validacao_modelo_sapi():
    ...
    while True:
        ...
        if carrega_valida_modelo(caminho_modelo, sapi_admim=True):
            print_tela()
        ...
        if not pergunta_sim_nao("< Deseja validar novamente o arquivo?", default="n"):
            print_tela_log("- Validação de modelo [", caminho_modelo, "] encerrada")
            return True
...
def eh_variavel_siscrim(campo):
    ...
    for nome_base_campo in lista_campos_siscrim:
        ...
        # Desta forma, aqui temos que checar se o campo está contido,
        # para desconsiderar o sufixo
        if nome_base_campo in campo:
...
def carrega_valida_modelo(caminho_arquivo_saida_odt, sapi_admim=False):
    ...
    Gfilho_para_pai_map = {c: p for p in office_text.iter() for c in p}
    ...
    (sucesso, Glista_cv_office_text) = odt_recupera_lista_campos_variaveis_sapi(office_text)
    ...
    (sucesso, Gdblocos) = carrega_blocos(office_text)
    if (not sucesso):
        return False
    ...
    if (not sucesso):
        print_tela_log("- ERRO: Corrija blocos de quesitação conforme indicado")
        return False


    # Modelo válido
    print_tela_log("- Modelo de laudo SAPI está sintaticamente correto")
    return True
...
def busca_quesitacao_similar(texto_solicitacao, quesitos_respostas):
    ...
    # Processa o texto com o mesmo dicionario obtido no processamento dos quesitos
    corpus.append(texto_solicitacao)
    vectorizer1 = CountVectorizer(vocabulary=vectorizer.vocabulary_)
    features = vectorizer1.fit_transform(corpus).todense()

    ix_menor=None
    menor=None
    ix=1
    for ix in range(1,len(features)-1):
        valor=euclidean_distances(features[len(features)-1], features[ix])[0][0]
...
def busca_quesitacao_similar_IA(texto_solicitacao, quesitos_respostas):
...
def main():
    ...
    if modo_config():
        ...
        # Efetua validação de um modelo de laudo SAPI
        validacao_modelo_sapi()
        print("- Finalizando configuração do sapi_laudo")
        return
