...
def exibirSecaoLaudo(d):
    print_ok("-" * 40, " Dados para laudo ", "-" * 40)
    ...
    for categoria in sorted(d):
        ...
        for seq in sorted(d[categoria]):
            ...
            print_ok("  ", d[categoria][seq].get("nome", "*** nome indefinido ***"),
            ...
            for prop in sorted(d[categoria][seq]):
                ...
                print_ok('    %12s : %s' % (prop, d[categoria][seq][prop]))
...
def exibirDadosLaudoItem(item):
    print_ok()
    print_ok("Exibição de dados de laudo para o item corrente")
    print_ok("- Recuperando dados do servidor. Aguarde...")
    ...
    if (not sucesso):
        # Continua no loop
        print_ok("Falha na busca de tarefas no servidor: ", msg_erro)
        return
    ...
    # ------------------------------------------------------------------
    dados_item = Gitens[itemCorrente()]
    print_ok()
    print_ok("-" * 129)
    print_ok("Item: ", dados_item["item_apreensao"])
    print_ok("Material: ", dados_item["material"])
    print_ok("Descrição: ", dados_item["descricao"])
    print_ok("-" * 129)
    ...
    for t in tarefas:
        ...
        exibirSecaoLaudo(dados_laudo)

    return


# Exibe dados de laudo para o item corrente
def exibirDadosLaudoItemCorrente():
    item = Gitens[itemCorrente()]["item"]
    # var_dump(item)
    # die('ponto609')
    exibirDadosLaudoItem(item)
...
def receberComando():
    ...
    cmdNavegacao = ["+", "-", "*ir"]
    cmdItem = ["*si"]  # *du - dump, não aparece na listagem
    cmdGeral = ["*sg", "*gl", "*qq"]

    comando_ok = False
    while not comando_ok:
        print_ok()
        entrada_original = input("Comando (?=ajuda): ")
        entrada = entrada_original.lower().strip()
        l = entrada.split(" ", 2)
        comando = ""
        if (len(l) >= 1):
            comando = l[0]
        ...
        if comando in comandos:
            ...
            for key in cmdNavegacao:
                print_ok(key.upper(), " : ", comandos[key])
            print_ok()
            ...
            if (comando != ""):
                print_ok("Comando (" + entrada_original + ") inválido")

    return (comando.lower(), argumento)
...
def odt_obtem_texto_recursivo(elem):
    ...
    for filho in elem:
        t = t + odt_obtem_texto_recursivo(filho)
...
def odt_obtem_texto_total_paragrafo(p):
	...
	t=""
	if (p.text!=None):
		# Texto no próprigo tag text:p
		t=t+p.text

	# Texto em tags text:span filhos de text:p
	for filho in p:
		if odt_get_tipo(filho)=='span':
			t=t+str(filho.text)
	'''
...
def odt_dump_paragrafo(p):
    ...
    print_ok(texto.encode("utf-8"))  # Gambiarra para conseguir exibir na console windows
...
def odt_procura_paragrafo_contendo_texto(elemento, texto):
    ...
    for p in elemento.findall('text:p', Gxml_ns):
        ...
        if (p.text != None) and (texto.lower() in p.text.lower()):
            return p
...
'''
# Recupera todos os parágrafos subordinados ao elemento
# retornando uma lista com dicionário:
#  - elemento=parágrafo
#  - Pai
# retorna o elemento textual que contém texto indicado
def odt_recupera_lista_paragrafos_deprecated(pai):
	...
	for filho in pai:
		...
		if (tipo=='p'):
			...
			dados["elemento"]=filho
			dados["pai"]=pai
			dados["texto"]=odt_obtem_texto_total_paragrafo(filho)
			dados["pos"]=pos
			lista.append(dados)
		else:
			# Se não for p, chama recursivamente, pois estamos
			# interessando em parágrafos existentes em qualquer nível
			lista_filho=odt_recupera_lista_paragrafos( filho)
			lista=lista+lista_filho
...
# </text:span>
#
def odt_substitui_campo_variavel_texto(base, lista_cv, substituir, valor):
    qtd_substituicoes = 0
    ...
    for cv in lista_cv:
        if (cv["name"].lower() == substituir.lower()):
            ...
            texto = odt_obtem_texto_total_paragrafo(pai)
            ...
            qtd_substituicoes = qtd_substituicoes + 1
...
def odt_dump(elem, nivel=0):
    ...
    # Exibe todos os ancestrais de um elemento
...
def carrega_blocos(base):
    ...
    nome_bloco = None
    blocoInicio = "SapiBlocoInicio"
    blocoFim = "SapiBlocoFim"
    q = 0
    for d in lista_p:
        q = q + 1
        # var_dump(d)
        ...
        if (texto.lower()[:len(blocoInicio)] == blocoInicio.lower()):
            ...
            # Verifica estado
            if (bloco_iniciado):
                print_tela_log("Bloco [", nome_bloco, "] foi iniciado mas não foi concluído (", blocoFim, ")")
                # Fracasso
                return (False, {})
            bloco_iniciado = True
            ...
            if dblocos.get(nome_bloco, None) != None:
        ...
        if (texto.lower()[:len(blocoFim)] == blocoFim.lower()):

            if (not bloco_iniciado):
                print_tela_log(
                    "- Bloco finalizado em ser iniciado. Procure no texto por um '" + blocoFim + "' sobrando")
                # Fracasso
                return (False, {})
...
def recupera_dados_para_laudo_das_tarefas_item(dadosItem, fase=None):
    listaDadosLaudo = []
    ...
    for t in tarefas:
        ...
        dados_relevantes_json = json.loads(t["tarefa"]["dados_relevantes_json"])
        dados_laudo = dados_relevantes_json["laudo"]
        listaDadosLaudo.append(dados_laudo)
    # var_dump(dados_laudo)
    # var_dump(dados_laudo)
    # die('ponto1292')
...
def criar_linha_para_item(tr_modelo, dadosItem, dblocos):
    ...
    # --------------------------------------------------------------
    dcs = {}
    dcs['sapiItem'] = dadosItem['item']
    dcs['sapiDescricaoSiscrim'] = dadosItem['descricao']
    dcs['sapiMaterialSiscrim'] = dadosItem['material']

    # Pode não existir lacre de entrada
    if (dadosItem['lacre_anterior'] != None):
        dcs['sapiLacre'] = dadosItem['lacre_anterior']

    for cs in dcs:
        odt_substitui_campo_variavel_texto(tr_nova_linha_item, lista_cv_tr_nova_linha_item, cs, dcs[cs])

    # Recupera dados para laudo tarefas do item da fase de aquisição
    listaDadosLaudo = recupera_dados_para_laudo_das_tarefas_item(dadosItem, fase='10-aquisicao')
    # var_dump(listaDadosLaudo)
    # die('ponto1318')

    for dadosTarefa in listaDadosLaudo:

        # --------------------------------------------------------------
        # Tratando sapiItemIdentificacao
        # --------------------------------------------------------------
        # var_dump(posicao_substituir)
        ...
        quantidade_componentes = dadosTarefa['sapiQuantidadeComponentes']
        print_log("Item possui", quantidade_componentes, " componentes")
        # var_dump(quantidade_componentes)
        # die('ponto1327')
        ...
        while (q_comp <= quantidade_componentes):
            ...
            dados_componente = dadosTarefa[ix_comp]
            tipo = dados_componente['sapiTipoComponente']
            # if (tipo=='sim'):
            #	var_dump(dados_componente)
            #	die('ponto1338')
            ...
            for par in bloco:
                ...
                for substituir in dados_componente:
                    ...
                    # var_dump(valor)
                    # die('ponto1370')
                    odt_substitui_campo_variavel_texto(novo_paragrafo, lista_cv, substituir, valor)
                # odt_dump(novo_paragrafo)
                # die('ponto1375')
...
def criar_linhas_hash_para_item(tr_modelo, dadosItem):
    # Retorno
    linhas = []

    # Recupera dados de laudo das tarefas do item
    listaDadosLaudo = recupera_dados_para_laudo_das_tarefas_item(dadosItem)
    ...
    for dadosTarefa in listaDadosLaudo:
        ...
        sapiHashes = dadosTarefa.get('sapiHashes', None)
        if (sapiHashes == None):
        ...
        for h in sapiHashes:
            ...
            descricao = "Item " + dadosItem['item'] + " - " + h["sapiHashDescricao"]
...
def ajustar_laudo_odt(caminho_arquivo_entrada_odt):
    ...
    (pasta_entrada, nome_arquivo_entrada) = decompoeCaminho(caminho_arquivo_entrada_odt)
    ...
    if (not sucesso):
        # Continua no loop
        print_ok("Falha na busca de dados da solicitação de exame no servidor: ", msg_erro)
        return
    ...
    for substituir in dsub:
        ...
        qtd_substituicoes = odt_substitui_campo_variavel_texto(odt_raiz, lista_cv_office_text, substituir, novo_valor)
    ...
    cv_sapiQuesitos = odt_localiza_campo_variavel(lista_cv_office_text, 'sapiQuesitos')
    if (cv_sapiQuesitos == None):
        print_tela_log("ERRO: Modelo sapi mal formado. Não foi localizado campo de substituição 'sapiQuesitos'.")
        return

    # var_dump(cv_sapiQuesitos)
    # die('ponto1831')

    paragrafo_substituir_sapiQuesitos = odt_busca_ancestral_com_tipo(cv_sapiQuesitos['pai'], raiz=odt_raiz, tipo='p')
    # var_dump(ancestral)
    # odt_dump(ancestral)
    # die('ponto1836')
    if (paragrafo_substituir_sapiQuesitos == None):
        print_tela_log("ERRO: Não encontrado parágrafo onde se localiza 'sapiQuesitos'")
        return

    # var_dump(paragrafo_substituir_sapiQuesitos)
    # die('ponto1860')

    paragrafo_substituir_pai = odt_substituir_paragrafo_por_lista(paragrafo_substituir_sapiQuesitos, odt_raiz,
                                                                  lista_par_quesitos)
    # odt_dump(paragrafo_substituir_pai)
    # die('ponto1865')
    ...
    # -------------------------------------------------------------------
    cv_sapiRespostas = odt_localiza_campo_variavel(lista_cv_office_text, GsapiRespostas)
    if (cv_sapiRespostas == None):
        print_tela_log(
            "ERRO: Modelo sapi mal formado. Não foi localizado campo de substituição '" + GsapiRespostas + "'.")
        return

    paragrafo_substituir_sapiRespostas = odt_busca_ancestral_com_tipo(cv_sapiRespostas['pai'], raiz=odt_raiz, tipo='p')
    if (paragrafo_substituir_sapiRespostas == None):
        print_tela_log("ERRO: Não encontrado parágrafo onde se localiza '" + GsapiRespostas + "'")
        return

    paragrafo_substituir_pai = odt_substituir_paragrafo_por_lista(paragrafo_substituir_sapiRespostas, odt_raiz,
                                                                  lista_par_respostas)
    ...
    for dadosItem in Gitens:
        ...
        for linha in linhas_hash:
            ...
            posicao_inserir_tab_hashes = posicao_inserir_tab_hashes + 1
    ...
    # var_dump(tab_materiais)
    # var_dump(posicao_inserir_tab_materiais)
    # odt_dump(Gxml_ns,tab_materiais)
    # die('ponto1251')
    ...
    for dadosItem in Gitens:
        ...
        posicao_inserir_tab_materiais = posicao_inserir_tab_materiais + 1
    ...
    geraNovoOdt(odt_raiz, caminho_arquivo_saida_odt)
...
def geraNovoOdt(odt_raiz, caminho_arquivo_saida_odt):
    #
    # Remove todos os comentários do documento gerado
    # Um comentário é um parágrafo iniciado por #
    odt_remove_comentarios(odt_raiz)
    ...
    # var_dump(xml_string_alterado)
    # die('ponto738')


    # Remove content.xml do zip
    componenteTrocar = "content.xml"
    remove_from_zip(caminho_arquivo_saida_odt, [componenteTrocar])
    ...
    with zipfile.ZipFile(caminho_arquivo_saida_odt, mode='a', compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr(componenteTrocar, xml_string_alterado)

    print_ok("Novo arquivo gravado em: ", caminho_arquivo_saida_odt)
    return
...
def gerarLaudo():
    ...
    print_ok("Em modo de desenvolvimento...desviando para teste1()")
    ...
    while True:
        ...
        while not selecionada_pasta:
            print_ok()
            print_ok("2) Modelo do Laudo")
            print_ok("- Entre no SisCrim e gere para o laudo informado um modelo SAPI.")
            print_ok("- Na janela gráfica que foi aberta, informe o arquivo .ODT do modelo do laudo.")

            # Cria janela para seleção de laudo
            root = tkinter.Tk()
            j = Janela(master=root)
            caminho_laudo = j.selecionarArquivo()
            root.destroy

            # Exibe a pasta obtida do clipboard, para usuário conferir
            print_ok("- Arquivo do modelo do laudo: ", caminho_laudo)
            print_ok()
...
def exibirSituacao():
    ...
    print_ok(GdadosGerais["identificacaoLaudo"])
    print_ok('-' * 129)
    ...
    for i in Gitens:
        ...
        # cabecalho
        if (q == 1):
            print_ok('%2s %2s %15s %-101.101s' % (" ", "Sq", "Material", "Item : Descrição"))
            print_ok('-' * 129)
        # Tarefa
        item_descricao = i["item"] + " : " + i["descricao"]
        print_ok('%2s %2s %15s %-101.101s' % (corrente, q, i["material"], item_descricao))
    ...
    print_ok()
    if (q == 0):
        print_ok("Não existe nenhum item pronto para laudo nesta solicitação de exame.")
    else:
        print_ok("A lista acima apresenta apenas os itens da solicitação de exame que estão prontos para laudo.")
    print_ok(
        "Um item está pronto para laudo quando todas as tarefas programadas para o mesmo foram concluídas com êxito.")
    print_ok("Em caso de dúvida, consulte o setec3")
    print_ok()
    return
...
def carregarEstado():
    ...
    if (not os.path.isfile(GarquivoEstado)):
        return

    # Le dados do arquivo e fecha
    arqEstado = open(GarquivoEstado, "r")
...
def obterLaudoItens():
    ...
    while True:
        ...
        if (matricula == ''):
            print_ok("Nenhuma matrícula informada. Programa finalizado.")
            sys.exit()

        if (not matricula.isdigit()):
            print_ok("Matrícula deve ser campo numérico (ex: 15123).")
            continue
        ...
        if (len(lista_laudos) == 0):
            print_ok("Não existe nenhum laudo para exame SAPI na carga da matrícula indicada. Verifique no SisCrim")
            continue
    ...
    for d in lista_laudos:
        ...
        if (q == 1):
            # Cabecalho
            print_ok('%2s  %10s %10s  %s' % ("N.", "Laudo", "Protocolo", "Solicitação de exame"))
            print_ok('-' * 129)
        protocolo_ano = d["numero_protocolo"] + "/" + d["ano_protocolo"]
        laudo_ano = d["numero_documento"] + "/" + d["ano_documento"]
        # var_dump(d)
        # die('ponto1961')
        print_ok('%2d  %10s %10s  %s' % (q, laudo_ano, protocolo_ano, d["identificacao"]))

    # print_ok("type(lista_laudos)=",type(lista_laudos))
    ...
    while True:
        ...
        if not num_solicitacao.isdigit():
            print_ok("Entre com o numero do laudo")
            continue

        # Verifica se existe na lista
        num_solicitacao = int(num_solicitacao)
        if not (num_solicitacao >= 1 and num_solicitacao <= len(lista_laudos)):
            # Número não é válido
            print_ok("Entre com o numero do laudo, entre 1 e ", str(len(lista_laudos)))
            continue
        ...
        # print_ok("Laudo:",solicitacao["identificacao"])
        # var_dump(GdadosGerais["identificacaoLaudo"])
        # die('ponto2001')

        print_ok("Buscando itens prontos para " + GdadosGerais["identificacaoLaudo"])
        print_ok("Aguarde...")
        ...
        if (len(itens) == 0):
            print_ok()
            print_ok("A solicitação de exame deste laudo NÃO TEM NENHUM ITEM PRONTO PARA LAUDO")
            print_ok("Verifique no SETEC3.")
            print_ok()
            continue
        ...
        for item in itens:
            ...
            if (q == 1):
                # Cabecalho
                print_ok('%10s %s' % ("item", "Descrição"))
                print_ok('-' * 129)
            print_ok('%10s %s' % (item["item"], item["descricao"]))

        print_ok()
        print_ok("Total de itens disponíveis para laudo: ", q)
        print_ok()
        print_ok()
...
def refreshItens():
    ...
    print_ok("Buscando situação atualizada do servidor. Aguarde...")
...
# ---------
print_ok()
print_ok(GnomePrograma, " (Versao " + Gversao + ")")
print_ok("=========================================")
print_ok()
print_log('Iniciando ', GnomePrograma, ' - ', Gversao)
...
# Recebe comandos
while (True):
    (comando, argumento) = receberComando()
    if (comando == '*qq'):
...
# Finaliza
print_ok()
print_ok("FIM ", GnomePrograma, " (Versão ", Gversao, ")")
