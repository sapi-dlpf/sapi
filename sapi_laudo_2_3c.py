...
Gdblocos                = None
Gfilho_para_pai_map     = None
Gquesitos_respostas     = None
...
def odt_extrai_content_xml(caminho_arquivo_saida_odt):
    ...
    try:
        ...
        print_tela_log("ERRO: Ao unzipar", caminho_arquivo_saida_odt)
...
def odt_carrega_arquivo(caminho_arquivo_odt):
    ...
    # Para debug
    print("="*100)
    print("Carregando arquivo: ", caminho_arquivo_odt)
    print("Arquivo de content.xml: ", caminho_arq_content_xml)
    #print("Início do arquivo:", xml_string[0:100])
    print()
    print("==== Namespace ====")

    # Primeirameite é necessário carrregar
    odt_print_ns(parse_and_get_ns(caminho_arq_content_xml))

    # Primeirameite é necessário carregar os name spaces
    ...
    odt_armazena_e_registral_ns(parse_and_get_ns(caminho_arq_content_xml))
    ...
    try:
        ...
        print_tela_log("ERRO: Problema no parse do arquivo", caminho_arq_content_xml)
        print_tela_log("ERRO: ", e)
        return False



# Exibe os name spaces identificados
...
def odt_armazena_e_registral_ns(xml_ns):
    ...
    for k in Gxml_ns:
        ...
        #debug("Namespace", prefix, "registrado para", uri)
        debug("Namespace", prefix, "registrado para", uri)
        qtd = qtd+1
...
def parse_and_get_ns(caminho_arq):
...
def ajustar_laudo_odt(caminho_arquivo_entrada_odt):
    ...
    #kkk
    #quesitacao_selecionada = sistema_escolhe_quesitacao(Gquesitos_respostas)

    # Usuário escolhe quesitação, em modo interativo
    quesitacao_selecionada = usuario_escolhe_quesitacao(Gquesitos_respostas)
    ...
    lista_par_quesitos = odt_clonar_bloco(nome_bloco_quesitos, Gdblocos)
    lista_par_respostas = odt_clonar_bloco(nome_bloco_respostas, Gdblocos)

    # ------------------------------------------------------------------------------------------------------------------
    # Substitui sapiQuesitos
    # ------------------------------------------------------------------------------------------------------------------
    cv_sapi_quesitos = odt_localiza_campo_variavel(Glista_cv_office_text, 'sapiQuesitos')
    ...
    # ------------------------------------------------------------------------------------------------------------------
    cv_sapi_respostas = odt_localiza_campo_variavel(Glista_cv_office_text, GsapiRespostas)
    ...
    # ---------------------------
    tabela_materiais = Gtabelas['tabela_materiais']
    ...
    tab_materiais = Gfilho_para_pai_map[tr_modelo]
    ...
    for dados_item in Gitens:
        ...
        tr_nova_linha_item = criar_linha_para_item(tr_modelo, dados_item, Gdblocos)
    ...
    for substituir in dsub:
        ...
        qtd_substituicoes = odt_substitui_campo_variavel_texto(Glista_cv_office_text, substituir, novo_valor)
    ...
    cv_sapi_entrega = odt_localiza_campo_variavel(Glista_cv_office_text, GsapiEntrega)
    ...
    while nova_variavel:
        ...
        for campo in lista_cv:
            ...
            if Gdblocos.get(nome_bloco_substituir, None) is None:
            ...
            lista_par_bloco = odt_clonar_bloco(nome_bloco_substituir, Gdblocos)
...
def atualiza_arquivo_laudo(odt_raiz, caminho_arquivo_saida_odt):
    ...
    return substitui_content_xml_em_odt(odt_raiz, caminho_arquivo_saida_odt)
...
def _validacao_modelo_sapi():
    ...
    while True:
        ...
        if validado_com_sucesso:
            ...
            print()
            if pergunta_sim_nao("< Deseja visualizar as opções de quesitos definidas no modelo?", default="n"):
                # Usuário escolhe e visualiza quesitação, em modo interativo
                _usuario_escolhe_quesitacao(Gquesitos_respostas, apenas_listar=True)

        # Continua validando
        print()
        if not pergunta_sim_nao("< Deseja validar novamente o arquivo?", default="n"):
            print_tela_log("- Validação do arquivo de modelo", caminho_modelo, "encerrada")
            break

    # Se foi feita alguma validação com sucesso
    if validado_com_sucesso:
        print()
        print("- Se você terminou a customização do modelo, crie/atualize o modelo de laudo da Unidade no SisCrim")
        if pergunta_sim_nao("< Deseja atualizar o modelo no Siscrim", default="s"):
...
def carrega_valida_modelo(caminho_arquivo_saida_odt, template=False):

    global Gdblocos
    global Gfilho_para_pai_map
    global Glista_cv_office_text
    global Godt_raiz
    global Gquesitos_respostas
    ...
    # Um comentário é um parágrafo iniciado por #
    # Deprecated
    # Isto agora é feito no final, se for para geração de laudo
    # Se for na geração de modelo a partir de template, os comentários ficam
    #odt_remove_comentarios(Godt_raiz)
    odt_busca_contantes(Godt_raiz)

    #var_dump(Gmodelo_configuracao)
    #print(Gmodelo_configuracao)
    if (Gmodelo_versao_esperado not in Gmod_constantes):
    ...
    Gfilho_para_pai_map = {c: p for p in Goffice_text.iter() for c in p}
    ...
    (sucesso, Glista_cv_office_text) = odt_recupera_lista_campos_variaveis_sapi(Goffice_text)
    #var_dump(lista_cv_office_text)
    #die('ponto1881')
    if (not sucesso):
        print_tela_log(
            "ERRO: Campos variáveis inválidos: Providencie o ajuste dos campos variáveis no modelo (consulte o administrador SAPI de sua unidade)")
        return False
    if (len(Glista_cv_office_text) == 0):
        print_tela_log(
            "ERRO: Não foi detectado nenhum campo variável no padrão sapi. Assegure-se de utilizar um modelo de ODT padrão SAPI")
        return False
    ...
    for nome_cv in cv_esperados:
        if odt_procura_cv_lista(Glista_cv_office_text, nome_cv) is None:
    ...
    (sucesso, Gdblocos) = carrega_blocos(Goffice_text, excluir_blocos)
    if (not sucesso):
        return False
    print("- Quantidade de blocos: ", len(Gdblocos))
    ...
    for nome_bloco in blocos_esperados:
        if odt_procura_bloco(Gdblocos, nome_bloco) is None:
            print_tela_log("- ERRO: Não foi encontrado bloco obrigatório:", nome_bloco)
            return False
        else:
    ...
    # Verifica quesitação
    # -------------------------------------------------------------------
    print_tela_log("- Validação de quesitação")
    (sucesso, Gquesitos_respostas) = parse_quesitos_respostas(Gdblocos)
    if not sucesso:
        return False
    qtd_quesitos = len(Gquesitos_respostas)
    print("- Quantidade de quesitos encontrados:", qtd_quesitos)
    if (qtd_quesitos==0):
        print("- ERRO: Não foi encontrado nenhum quesito")
        return False
    for nome_quesito in Gquesitos_respostas:
        print("- Quesito com identificação:", nome_quesito)

    # -------------------------------------------------------------------
    # Verifica tabelas
    # -------------------------------------------------------------------
    print_tela_log("- Verificando existência de tabelas")
    Gtabelas = dict()
    ...
    for table in Goffice_text.findall('table:table', Gxml_ns):
        # print(table)
        # print(table.attrib)
        # die('ponto643')
        nome_tabela = obtem_atributo_xml(table, 'table:name')
    ...
    for nome_tabela in tabelas_esperadas:
        if tabelas_esperadas[nome_tabela]==0:
            ...
            print("- Encontrada tabela", nome_tabela)

    # ------------------------------------------------------------------
    # valida tabela de materiais
    # ------------------------------------------------------------------
    print("- Valida tabela_materiais")
    tabela_materiais = Gtabelas['tabela_materiais']
    tr_linhas = tabela_materiais.findall('table:table-row', Gxml_ns)
    ...
    #x=odt_recupera_recursivamente(tr_modelo, 'user-field-get')
    #var_dump(x)
    #die('ponto3251')
    ...
    # Modelo válido
    print_tela_log("- OK. Modelo de laudo SAPI está sintaticamente correto")
    return True
...
def _gerar_modelo_unidade():
    ...
    # Cabeçalho
    cls()
    print("Gerar modelo de laudo SAPI da unidade")
    print("=====================================")
    ...
    while True:
        ...
        print()
        print("- Agora será efetuado um acesso ao SAPI WEB para filtrar os quesitos conforme solicitado.")
        print("- Isto pode demorar alguns minutos, e não haverá nenhum indicativo de progresso durante a busca.")
        if not prosseguir():
            return False
        ...
        print("- Recuperando quesitos repetidos. Aguarde...")
        quesitacoes = sapisrv_chamar_programa_sucesso_ok(
        ...
        if qtd>0:
            print_tela_log("- Com os parâmetros informados foram recuperados", qtd, "quesitos")
            break
    ...
    criar_modelo_unidade(caminho_template, quesitacoes)
...
class DialogoParametrosBuscaLaudo(tkinter.Frame):
    ...
    def initUI(self):
        ...
        ttk.Label(self, text="para ser considerado padrão (incluído no modelo):").pack(anchor=tkinter.W)
...
def execucao_admin():
    ...
    print()
    print("-"*100)
    print("- Você está em modo ADMINISTRADOR. Escolha o comando a ser executado.")
    print("-"*100)
...
# -------------------------------------------------------
# ----------------- Para testes -------------------------
Gteste_quesitos_respostas = {'cac_10': {'nome_bloco_quesitos': 'sapiquesitos-cac_10',
                                        'nome_bloco_respostas': 'sapirespostas-cac_10',
                                        'quantidade_quesitos': 10,
                                        'resumo_quesitos': 'I. Qual a natureza dos equipamentos '
                                                           'apresentados para exame?\n'
                                                           'II. Quais os números telefônicos de '
                                                           'habilitação dos aparelhos questionados?\n'
                                                           'III. É possível determinar a quem '
                                                           'pertencem?\n'
                                                           'IV. Quais os números discados constantes '
                                                           'na memória dos aparelhos?\n'
                                                           'V. Quais as chamadas recebidas '
                                                           'constantes na memória dos aparelhos?\n'
                                                           'VI. Qual a relação de nomes e telefones '
                                                           'constantes na agenda eletrônica dos '
                                                           'aparelhos?\n'
                                                           'VII. Quais as mensagens de texto '
                                                           'enviadas constantes na memória dos '
                                                           'aparelhos?\n'
                                                           'VIII. Quais as mensagens de texto '
                                                           'recebidas constantes na memória dos '
                                                           'aparelhos?\n'
                                                           'IX. Existem chamadas ou mensagens de '
                                                           'texto trocadas entre os numeros '
                                                           'constantes em cada um dos aparelhos? '
                                                           '(cruzamento de dado...\n'
                                                           'X. Outros dados julgados úteis.'},
                             'cac_12': {'nome_bloco_quesitos': 'sapiquesitos-cac_12',
                                        'nome_bloco_respostas': 'sapirespostas-cac_12',
                                        ...
                                        'resumo_quesitos': 'I. Qual a natureza dos equipamentos '
                                                           ...
                                                           'aparelhos?\n'
                                                           'IX. Existem chamadas ou mensagens de '
                                                           'texto, inclusive apagadas, trocadas '
                                                           'entre os numerais constantes em cada um '
                                                           'dos aparelho...\n'
                                                           'X. Exitem conversas, imagens, vídeos e '
                                                           'aúdios apagados do aplicativo '
                                                           '?whatsapp??\n'
                                                           'XI. Existem fotografias e vídeos em '
                                                           'mídias que foram apagadas da memória do '
                                                           'aparelho?\n'
                                                           'XII. Outros dados julgados úteis.'},
                             'cac_7': {'nome_bloco_quesitos': 'sapiquesitos-cac_7',
                                       'nome_bloco_respostas': 'sapirespostas-cac_7',
                                       'quantidade_quesitos': 7,
                                       'resumo_quesitos': '1) Características do aparelho e registro '
                             ...
                             'fig_7': {'nome_bloco_quesitos': 'sapiquesitos-fig_7',
                                       'nome_bloco_respostas': 'sapirespostas-fig_7',
                                       'quantidade_quesitos': 7,
                                       'resumo_quesitos': 'I. Qual a natureza do equipamento '
                                                          'apresentado para exame?\n'
                                                          'II. Qual o número telefônico de '
                                                          'habilitação do aparelho questionado?\n'
                                                          'III. É possível determinar a quem '
                                                          'pertence?\n'
                                                          'IV. Quais os números discados constantes '
                                                          'na memória do aparelho?\n'
                                                          'V. Quais as chamadas recebidas constantes '
                                                          'na memória do aparelho?\n'
                                                          'VI. Qual a relação de nomes e telefones '
                                                          'constantes na agenda eletrônica?\n'
                                                          'VII. Outros dados julgados úteis.'},
                             'geralduplicacaoextracao': {'nome_bloco_quesitos': 'sapiquesitos-geralduplicacaoextracao',
                                                         ...
                                                         'resumo_quesitos': '[...] solicito a Vossa '
                                                                            ...
                                                                            'análise, os disco...'},
                             'gpb_5': {'nome_bloco_quesitos': 'sapiquesitos-gpb_5',
                                       'nome_bloco_respostas': 'sapirespostas-gpb_5',
                                       'quantidade_quesitos': 5,
                                       'resumo_quesitos': '1. Qual a natureza e características '
                                                          'do(s) aparelho(s) de telefone '
                                                          'celular/chip(s) submetido(s) a exame?...\n'
                                                          '2. Qual o número habilitado no '
                                                          'aparelho/chip submetido a exame? \n'
                                                          '3. Quais os números de telefone, datas e '
                                                          'horas constantes dos registros das '
                                                          'últimas ligações efetuadas e recebidas '
                                                          'por tal(is...\n'
                                                          '4. Quais os nomes e números de telefone '
                                                          'constantes da(s) agenda(s) telefônica(s) '
                                                          'de tal(is) aparelho(s)/chip(s)?...\n'
                                                          '5. Outros dados julgados úteis.'},
                             'gra_5': {'nome_bloco_quesitos': 'sapiquesitos-gra_5',
                                       'nome_bloco_respostas': 'sapirespostas-gra_5',
                                       'quantidade_quesitos': 5,
                                       'resumo_quesitos': '1. Qual a natureza e características '
                                                          'do(s) aparelho(s) de telefone '
                                                          'celular/chip(s) submetido(s) a exame?...\n'
                                                          ...
                                                          'de tal(is) aparelho(s)/chip(s)?...\n'
                                                          '5. Outros dados julgados úteis.'},
                             'lda_5': {'nome_bloco_quesitos': 'sapiquesitos-lda_5',
                                       'nome_bloco_respostas': 'sapirespostas-lda_5',
                                       'quantidade_quesitos': 5,
                                       'resumo_quesitos': '1. Qual a natureza e características '
                                                          'do(s) aparelho(s) de telefone celular e '
                                                          'chip(s) submetido(s) a exame?...\n'
                                                          '2. Qual o número habilitado no(s) '
                                                          'aparelho(s) submetido a exame? \n'
                                                          '3. Quais os números de telefone, datas e '
                                                          'horas constantes dos registros das '
                                                          'últimas ligações efetuadas e recebidas '
                                                          'por tal(is...\n'
                                                          ...
                                                          'de tal(is) aparelho(s)/chip(s)?...\n'
                                                          '5. Outros dados julgados úteis.'},
                             'lda_6': {'nome_bloco_quesitos': 'sapiquesitos-lda_6',
                                       'nome_bloco_respostas': 'sapirespostas-lda_6',
                                       'quantidade_quesitos': 6,
                                       'resumo_quesitos': '1. Qual a natureza e características do '
                                                          'aparelho de telefone celular e chip '
                                                          'submetido a exame?\n'
                                                          '2. Qual o número habilitado no aparelho '
                                                          'submetido a exame?\n'
                                                          '3. Quais os números de telefone, datas e '
                                                          'horas constantes dos registros das '
                                                          'últimas ligações efetuadas e recebidas '
                                                          'por tal ap...\n'
                                                          '4. Quais os nomes e números de telefone '
                                                          'constantes da agenda telefônica de tal '
                                                          'aparelho/chip?\n'
                                                          '5. Quais os arquivos existentes e sua '
                                                          'descrição (mensagens, etc) no aparelho e '
                                                          'chip?\n'
                                                          '6. Outros dados julgados úteis.'},
                             'lda_6_a': {'nome_bloco_quesitos': 'sapiquesitos-lda_6_a',
                                         'nome_bloco_respostas': 'sapirespostas-lda_6_a',
                                         'quantidade_quesitos': 6,
                                         'resumo_quesitos': '1. Qual a natureza e características '
                                                            'dos aparelhos de telefonia celular '
                                                            'submetidos a exame?\n'
                                                            '2. Quais os números habilitados nos '
                                                            'aparelhos submetidos a exame?\n'
                                                            '3. Quais os números de telefone, datas '
                                                            'e horas constantes dos registros das '
                                                            'últimas ligações efetuadas e recebidas '
                                                            'por tais a...\n'
                                                            '4. Quais os nomes e números de telefone '
                                                            'constantes das agendas telefônicas de '
                                                            'tais aparelhos?\n'
                                                            '5. Quais as mensagens recebidas e '
                                                            'encaminhadas?\n'
                                                            '6. Outros dados julgados úteis.'},
                             ...
                             'respostagenerica': {'nome_bloco_quesitos': 'sapiquesitos-respostagenerica',
                                                  ...
                                                  'quantidade_quesitos': 1,
                                                  'resumo_quesitos': '[?] solicitação genérica sem '
                                                                     'quesitos. Transcrever '
                                                                     'solicitação ou omitir solicito '
                                                                     'a realização de exame pericial '
                                                                     'visando ext...'},
                             'srpr_6': {'nome_bloco_quesitos': 'sapiquesitos-srpr_6',
                                        'nome_bloco_respostas': 'sapirespostas-srpr_6',
                                        'quantidade_quesitos': 6,
                                        'resumo_quesitos': '1. Qual a natureza e características '
                                                           'do(s) material(ais) submetido(s) a exame '
                                                           '?\n'
                                                           '2. Quais os números telefônicos de '
                                                           'habilitação associados?\n'
                                                           '3. Quais os registros correspondentes às '
                                                           'chamadas telefônicas recebidas, '
                                                           'efetuadas e não atendidas?\n'
                                                           '4. Quais os registros constantes da(s) '
                                                           'agenda(s) telefônica(s)?\n'
                                                           '5. Extrair os dados e metadados '
                                                           'relativos às comunicações eletrônicas '
                                                           '(por exemplo WhatsApp) eventualmente '
                                                           'armazenadas nos di...\n'
                                                           '6. Outros dados julgados úteis.'},
                             'srpr_9': {'nome_bloco_quesitos': 'sapiquesitos-srpr_9',
                                        'nome_bloco_respostas': 'sapirespostas-srpr_9',
                                        'quantidade_quesitos': 9,
                                        'resumo_quesitos': '1. Qual a natureza e características '
                                                           'do(s) material(ais) submetido(s) a '
                                                           'exame?\n'
                                                           '2. Quais os números telefônicos de '
                                                           'habilitação associados?\n'
                                                           '3. Há registros correspondentes às '
                                                           'chamadas telefônicas recebidas, '
                                                           'efetuadas e não atendidas? Caso '
                                                           'positivo, relacionar....\n'
                                                           '4. Há registros constantes da(s) '
                                                           'agenda(s) telefônica(s)? Caso positivo, '
                                                           'relacionar.\n'
                                                           '5. Existem registros referentes a '
                                                           'mensagens SMS? Caso positivo, '
                                                           'relacionar.\n'
                                                           '6. Há informações apagadas que puderam '
                                                           'ser recuperadas?\n'
                                                           '7. Extrair os dados e metadados '
                                                           'relativos às comunicações eletrônicas '
                                                           '(por exemplo WhatsApp), fotos e vídeos '
                                                           'eventualmente ar...\n'
                                                           '8. É possível, a partir dos dados '
                                                           'constantes no objeto de exame, '
                                                           'identificar o usuário do aparelho '
                                                           'celular?...\n'
                                                           '9. Outros dados julgados úteis.'},
                             'srpr_blackberry_11': {'nome_bloco_quesitos': 'sapiquesitos-srpr_blackberry_11',
                                                    'nome_bloco_respostas': 'sapirespostas-srpr_blackberry_11',
                                                    'quantidade_quesitos': 11,
                                                    'resumo_quesitos': '1. Qual a natureza e '
                                                                       'características do(s) '
                                                                       'material(ais) submetido(s) a '
                                                                       'exame?\n'
                                                                       '2. Quais os números '
                                                                       'telefônicos de habilitação '
                                                                       'associados?\n'
                                                                       '3. Há registros '
                                                                       ...
                                                                       'Caso positivo, '
                                                                       'relacionar....\n'
                                                                       '4. Há registros constantes '
                                                                       'da(s) agenda(s) '
                                                                       'telefônica(s)? Caso '
                                                                       'positivo, relacionar.\n'
                                                                       '5. Existem registros '
                                                                       'referentes a mensagens SMS? '
                                                                       'Caso positivo, relacionar.\n'
                                                                       '6. Há informações apagadas '
                                                                       'que puderam ser '
                                                                       'recuperadas?\n'
                                                                       '7. Extrair os dados e '
                                                                       'metadados relativos às '
                                                                       'comunicações eletrônicas '
                                                                       '(por exemplo WhatsApp), '
                                                                       'fotos e vídeos eventualmente '
                                                                       'ar...\n'
                                                                       '8. É possível, a partir dos '
                                                                       'dados constantes no objeto '
                                                                       'de exame, identificar o '
                                                                       'usuário do aparelho '
                                                                       'celular?...\n'
                                                                       '9. É possível realizar '
                                                                       'despejo (dump) físico da '
                                                                       'memória interna dos '
                                                                       'dispositivos?\n'
                                                                       '10. Em caso positivo, é '
                                                                       'possível a recuperação de '
                                                                       'mensagens associadas ao '
                                                                       'aplicativo BlackBerry '
                                                                       'Messenger?...\n'
                                                                       '11. Outros dados julgados '
                                                                       'úteis.'},
...
def busca_quesitacao_similar(texto_solicitacao, quesitos_respostas):

    # Monta lista de textos para comparação
    corpus=list()
    # O primeiro texto é o texto da solicitação
    #corpus.append(texto_solicitacao)
    # Os demais correspondem a cada um dos quesitos disponíveis
    ix = 0
    hits=dict()
    for qr in quesitos_respostas:
        var_dump(quesitos_respostas[qr])
        die('ponto5213')
        quesitos=quesitos_respostas[qr]['resumo_quesitos']
        hits[qr]=conta_hits(quesitos, texto_solicitacao)
    die('ponto3199')

    var_dump(hits)
    die('ponto3194')

    # Quesito mais proximo
    k_selecionada=corpus2quesitos[ix_menor]
    quesitos=quesitos_respostas[corpus2quesitos[ix_menor]]['resumo_quesitos']
    print()
    print("      menor valor: ",menor)
    print("ix do menor valor: ",ix_menor)
    print("K do ix          : ", k_selecionada)
    print(quesitos)
    print("-"*100)
    print(texto_solicitacao)
    die('pont3437')
...
def teste():
    ...
    busca_quesitacao_similar(Gteste_texto_solicitacao_exame_inedita, Gteste_quesitos_respostas)
...
# Retorna o caminho para o arquivo de saída
# ----------------------------------------------------------------------------------------------------------------------
def criar_modelo_unidade(caminho_arquivo_entrada_odt, quesitacoes):
    ...
    # ---------------------------------------------------------------------------------------------------
    (pasta_entrada, nome_arquivo_entrada) = path, file = os.path.split(caminho_arquivo_entrada_odt)
    nome_arquivo_saida = "laudo_modelo_sapi_unidade.odt"
    caminho_arquivo_saida_odt = os.path.join(pasta_entrada, nome_arquivo_saida)

    print_tela_log("- Arquivo de modelo da unidade será gravado em:", caminho_arquivo_saida_odt)
    if os.path.isfile(caminho_arquivo_saida_odt):
        if not pergunta_sim_nao("< Arquivo de saída já existe. Será substituido. Prosseguir?", default="n"):
            print("- Operação cancelada pelo usuário.")
            return
    ...
    # ------------------------------------------------------------------------------------------------------------------
    print()
    print("Passo 1: Validação de arquivo de template")
    print("-----------------------------------------")
    if not carrega_valida_modelo(caminho_arquivo_saida_odt, template=True):
        print("- Validação de modelo falhou")
        return False
    ...
    # ------------------------------------------------------------------------------------------------------------------
    print()
    print("Passo 2: Montagem de tabelas de quesitações padrões")
    print("---------------------------------------------------")

    # Tabela de quesitações
    # ---------------------------
    tabela_quesitacoes = Gtabelas['tabela_quesitacoes']

    # Isola linha de modelo da tabela de materiais
    tr_modelo = tabela_quesitacoes.findall('table:table-row', Gxml_ns)[0]

    #print()
    #odt_dump(tr_modelo)

    tab_quesitacoes = Gfilho_para_pai_map[tr_modelo]
    posicao_inserir_tab_quesitacoes = odt_determina_posicao_pai_filho(tab_quesitacoes, tr_modelo)
    ...
    for quesitacao in quesitacoes:
        ...
        #var_dump(quesitacao)
        #die('ponto1863')
        print_tela_log("- Tabela de quesitacoes, processando quesitação ", quesitacao['hash'])

        # Processa item, criando nova linha na tabela de quesitacoes
        tr_nova_linha_item = criar_linha_quesitacao_padrao(tr_modelo, quesitacao)
        if tr_nova_linha_item is None:
            return
    ...
    # ------------------------------------------------------------------------------------------------------------------
    print()
    print("Passo 3: Customização do laudo")
    print("------------------------------")

    # Gera novo documento e encerra
    if not substitui_content_xml_em_odt(Godt_raiz, caminho_arquivo_saida_odt):
        print("- Comando cancelado")
        return

    # Tudo certo, pergunta se usuário quer abrir arquivo
    print()
    print("- Modelo de laudo para a unidade gravado em: ", caminho_arquivo_saida_odt)
    if pergunta_sim_nao("< Deseja abrir o modelo para edição?"):
        webbrowser.open(caminho_arquivo_saida_odt)
        print("- Modelo foi aberto em programa compatível (normalmente libreoffice)")
        print("- Recomenda-se utilizar o LIBREOFFICE com versão superior a 5.2")

    print("- Leia e customize a modelo de laudo gerado, ajustando os textos conforme a necessidade da sua unidade")
    print("- Durante o ajuste é recomendável que você efetue verificações regulares da integridade do modelo.")
    if pergunta_sim_nao("< Deseja entrar em modo de validação do modelo (*VM)?"):
        carrega_valida_modelo(caminho_arquivo_saida_odt, template=True)
...
def criar_linha_quesitacao_padrao(tr_modelo_linha_quesitacao, quesitacao):
    ...
    bloco = Gdblocos.get(nome_bloco_componente.lower(), None)
    if bloco is None:
        print_tela_log(
            texto("Erro: Não foi localizado bloco com nome[",
                  nome_bloco_componente,
...
def obter_lista_respostas_padroes(texto_quesito):
    ...
    resposta['sapiRespostaIdentificarUsuario'] = ['julgados úteis', 'usuário']
    resposta['sapiRespostaIdentificarProprietario'] = ['a quem pertence', 'pertence']
    ...
    resposta['sapiRespostaArquivos'] = ['arquivos', 'áudios', 'vídeos']
...
def substitui_content_xml_em_odt(odt_raiz, caminho_arquivo_saida_odt):
...
def teste1():
    ...
    # modo_cp65001()
    # Utilizado para evitar que o var_dump aborte por problema de unicode

    #
    # sapisrv_inicializar_ok(Gprograma, Gversao, auto_atualizar=True)
    #
    # unidade=3547 # SETEC/PR
    # #unidade=3641 # SETEC/RN
    # #codigo_documento_interno=33566506
    # data_inicial = '2017-01-01'
    # data_final = '2017-12-31'
    # qtd_minima = 3
    #
    # # --- Por enquanto, vamos substituir a chamada por um valor já definido
    # print("- Recuperando quesitações padrões. Isto pode demorar alguns minutos. Aguarde...")
    # quesitacoes = sapisrv_chamar_programa_sucesso_ok(
    #     "sapisrv_obter_quesitacao_padrao.php",
    #      {'unidade':         unidade,
    #       #'codigo_documento_interno': codigo_documento_interno, #Somente para DEBUG
    #       'data_inicial':    data_inicial,
    #       'data_final':      data_final,
    #       'qtd_minima':      qtd_minima})
    #
    # var_dump(quesitacoes)
    # die('ponto5774')

    # Setec-PR, 2017-01-01 a 2017-01-31
    quesitacoes = [
    {   'codigo_documento_interno': '32496472',
        'hash': 'a4df41a4a845ab7bf95d8e5cbed26940',
        'identificacao': 'Laudo 163/2017-SETEC/SR/PF/PR',
        'ocorrencias': 5,
        'ocorrencias_codigo_unidade': {'3547': 5},
        'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 5},
        'quesitos': [   '1. Qual a natureza e características do (s) '
                        'aparelho(s) de telefone celular/chip(s) submetido(s) '
                        'a exame ?',
                        '2. Qual o número habilitado no aparelho/chip '
                        'submetido a exame?',
                        '3. Quais os números de telefone, datas e horas '
                        'constantes dos registros das últimas ligações '
                        'efetuadas e recebidas por tal (is) aparelho(s) de '
                        'telefonia celular/chip(s)?',
                        '4. Quais os nomes e números de telefone constantes da '
                        '(s) agenda(s) telefônica(s) de tal(is) '
                        'aparelho(s)/chip(s)?',
                        '5. Outros dados julgados úteis.'],
        'tamanho': 495,
        'texto': '1. Qual a natureza e características do (s) aparelho(s) de '
                 'telefone celular/chip(s) submetido(s) a exame ? 2. Qual o '
                 'número habilitado no aparelho/chip submetido a exame? 3. '
                 'Quais os números de telefone, datas e horas constantes dos '
                 'registros das últimas ligações efetuadas e recebidas por tal '
                 '(is) aparelho(s) de telefonia celular/chip(s)? 4. Quais os '
                 'nomes e números de telefone constantes da (s) agenda(s) '
                 'telefônica(s) de tal(is) aparelho(s)/chip(s)? 5. Outros '
                 'dados julgados úteis.'},
    {   'codigo_documento_interno': '32274411',
        'hash': '4545c3a5a10f777131339221604a7dac',
        'identificacao': 'Laudo 017/2017-SETEC/SR/PF/PR',
        'ocorrencias': 4,
        'ocorrencias_codigo_unidade': {'3547': 4},
        'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 4},
        'quesitos': [   '1. Qual a natureza e características do(s) '
                        'aparelho(s) de telefone celular/chip(s) submetido(s) '
                        'a exame?',
                        '2. Qual o número habilitado no aparelho/chip '
                        ...
                        'telefonia celular/chip(s)?',
                        '4. Quais os nomes e números de telefone constantes da '
                        'agenda (s) telefônica(s) de tal(is) '
                        'aparelho(s)/chip(s)?',
                        '5. Outros dados julgados úteis.'],
        'tamanho': 490,
        'texto': '1. Qual a natureza e características do(s) aparelho(s) de '
                 ...
                 'registros das últimas ligações efetuadas e recebidas por tal '
                 '(is) aparelho(s) de telefonia celular/chip(s)? 4. Quais os '
                 'nomes e números de telefone constantes da agenda (s) '
                 'telefônica(s) de tal(is) aparelho(s)/chip(s)? 5. Outros '
                 'dados julgados úteis.'}]

    # Setec-PR, 2017-01-01 a 2017-12-31
    quesitacoes = [
     {'codigo_documento_interno': '33913791',
      'hash': '4ab331f21e078731e560eaec9ad121fd',
      'identificacao': 'Laudo 891/2017-SETEC/SR/PF/PR',
      'ocorrencias': 6,
      'ocorrencias_codigo_unidade': {'3547': 6},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 6},
      'quesitos': ['1. Qual a natureza e características do (s) '
                   'material(ais) submetido(s) a exame?',
                   '2. Quais os números telefônicos de habilitação '
                   'associados?',
                   '3. Há registros correspondentes às chamadas '
                   'telefônicas recebidas, efetuadas e não atendidas? '
                   'Caso positivo, relacionar.',
                   '4. Há registros constantes da(s) agenda(s) '
                   'telefônica(s)? Caso positivo, relacionar.',
                   '5. Existem registros referentes a mensagens SMS? Caso '
                   'positivo, relacionar.',
                   '6. Há informações apagadas que puderam ser '
                   'recuperadas?',
                   '7. Extrair os dados e metadados relativos às '
                   'comunicações eletrônicas (por exemplo WhatsApp), '
                   'fotos e vídeos eventualmente armazenadas nos '
                   'dispositivos informáticos, devendo a apresentação '
                   'destes dados ser em formato de texto legível e '
                   'navegável.',
                   '8. É possível, a partir dos dados constantes no '
                   'objeto de exame, identificar o usuário do aparelho '
                   'celular?',
                   '9. Outros dados julgados úteis.'],
      'tamanho': 891,
      'texto': '1. Qual a natureza e características do (s) material(ais) '
               'submetido(s) a exame? 2. Quais os números telefônicos de '
               'habilitação associados? 3. Há registros correspondentes às '
               'chamadas telefônicas recebidas, efetuadas e não atendidas? '
               'Caso positivo, relacionar. 4. Há registros constantes da(s) '
               'agenda(s) telefônica(s)? Caso positivo, relacionar. 5. '
               'Existem registros referentes a mensagens SMS? Caso positivo, '
               'relacionar. 6. Há informações apagadas que puderam ser '
               'recuperadas? 7. Extrair os dados e metadados relativos às '
               'comunicações eletrônicas (por exemplo WhatsApp), fotos e '
               'vídeos eventualmente armazenadas nos dispositivos '
               'informáticos, devendo a apresentação destes dados ser em '
               'formato de texto legível e navegável. 8. É possível, a '
               'partir dos dados constantes no objeto de exame, identificar '
               'o usuário do aparelho celular? 9. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '33919542',
      'hash': '0dad2a914c6af8a22ca842873d3b6527',
      'identificacao': 'Laudo 895/2017-SETEC/SR/PF/PR',
      'ocorrencias': 13,
      'ocorrencias_codigo_unidade': {'3547': 13},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 13},
      'quesitos': ['I. Qual a natureza dos equipamentos apresentados para '
                   'exame?',
                   'II. Quais os números telefônicos de habilitação dos '
                   'aparelhos questionados?',
                   'III. É possível determinar a quem pertencem?',
                   'IV. Quais os números discados constantes na memória '
                   'dos aparelhos?',
                   'V. Quais as chamadas recebidas constantes na memória '
                   'dos aparelhos?',
                   'VI. Qual a relação de nomes e telefones constantes na '
                   'agenda eletrônica dos aparelhos?',
                   'VII. Quais as mensagens de texto enviadas constantes '
                   'na memória dos aparelhos?',
                   'VIII. Quais as mensagens de texto recebidas '
                   'constantes na memória dos aparelhos?',
                   'IX. Existem chamadas ou mensagens de texto trocadas '
                   'entre os numerais constantes em cada um dos '
                   'aparelhos? (cruzamento de dados)',
                   'X. Outros dados julgados úteis.'],
      'tamanho': 739,
      'texto': 'I. Qual a natureza dos equipamentos apresentados para exame? '
               'II. Quais os números telefônicos de habilitação dos '
               'aparelhos questionados? III. É possível determinar a quem '
               'pertencem? IV. Quais os números discados constantes na '
               'memória dos aparelhos? V. Quais as chamadas recebidas '
               'constantes na memória dos aparelhos? VI. Qual a relação de '
               'nomes e telefones constantes na agenda eletrônica dos '
               'aparelhos? VII. Quais as mensagens de texto enviadas '
               'constantes na memória dos aparelhos? VIII. Quais as '
               'mensagens de texto recebidas constantes na memória dos '
               'aparelhos? IX. Existem chamadas ou mensagens de texto '
               'trocadas entre os numerais constantes em cada um dos '
               'aparelhos? (cruzamento de dados) X. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '32496472',
      'hash': 'a4df41a4a845ab7bf95d8e5cbed26940',
      'identificacao': 'Laudo 163/2017-SETEC/SR/PF/PR',
      'ocorrencias': 9,
      'ocorrencias_codigo_unidade': {'3547': 9},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 9},
      'quesitos': ['1. Qual a natureza e características do (s) '
                   'aparelho(s) de telefone celular/chip(s) submetido(s) '
                   'a exame ?',
                   '2. Qual o número habilitado no aparelho/chip '
     ...
     {'codigo_documento_interno': '32274411',
      ...
      'texto': '1. Qual a natureza e características do(s) aparelho(s) de '
               ...
               'telefônica(s) de tal(is) aparelho(s)/chip(s)? 5. Outros '
               'dados julgados úteis.'},
     {'codigo_documento_interno': '32637103',
      'hash': 'f3b3fe1939ce6efeff40099f6f8f235d',
      'identificacao': 'Laudo 217/2017-SETEC/SR/PF/PR',
      'ocorrencias': 8,
      'ocorrencias_codigo_unidade': {'3547': 8},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 8},
      'quesitos': ['I. Qual a natureza do equipamento apresentado para '
                   'exame?',
                   'II. Qual o número telefônico de habilitação do '
                   'aparelho questionado?',
                   'III. É possível determinar a quem pertence?',
                   'IV. Quais os números discados constantes na memória '
                   'do aparelho?',
                   'V. Quais as chamadas recebidas constantes na memória '
                   'do aparelho?',
                   ...
                   'VII. Outros dados julgados úteis.'],
      'tamanho': 421,
      'texto': 'I. Qual a natureza do equipamento apresentado para exame? '
               'II. Qual o número telefônico de habilitação do aparelho '
               'questionado? III. É possível determinar a quem pertence? IV. '
               'Quais os números discados constantes na memória do aparelho? '
               'V. Quais as chamadas recebidas constantes na memória do '
               'aparelho? VI. Qual a relação de nomes e telefones constantes '
               'na agenda eletrônica? VII. Outros dados julgados úteis.'},
     {'codigo_documento_interno': '36989447',
      'hash': 'd274a420c3a79f1c9815f78510892502',
      'identificacao': 'Laudo 2148/2017-SETEC/SR/PF/PR',
      'ocorrencias': 68,
      'ocorrencias_codigo_unidade': {'3547': 68},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 68},
      'quesitos': ['1. Qual a natureza e características do(s) '
                   'material(ais) submetido(s) a exame?',
                   '2. Quais os números telefônicos de habilitação '
                   'associados?',
      ...
      'texto': '1. Qual a natureza e características do(s) material(ais) '
               ...
               'o usuário do aparelho celular? 9. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '36206592',
      'hash': '4d8f07522192b173405fad9fead7f42e',
      'identificacao': 'Laudo 1802/2017-SETEC/SR/PF/PR',
      'ocorrencias': 11,
      'ocorrencias_codigo_unidade': {'3547': 11},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 11},
      'quesitos': ['I. Qual a natureza dos equipamentos apresentados para '
                   'exame?',
                   'II. Quais os números telefônicos de habilitação dos '
                   'aparelhos questionados?',
                   ...
                   'constantes na memória dos aparelhos?',
                   'IX. Existem chamadas ou mensagens de texto trocadas '
                   'entre os numeros constantes em cada um dos aparelhos? '
                   '(cruzamento de dados)',
                   'X. Outros dados julgados úteis.'],
      'tamanho': 738,
      'texto': 'I. Qual a natureza dos equipamentos apresentados para exame? '
               ...
               'mensagens de texto recebidas constantes na memória dos '
               'aparelhos? IX. Existem chamadas ou mensagens de texto '
               'trocadas entre os numeros constantes em cada um dos '
               'aparelhos? (cruzamento de dados) X. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '37249028',
      'hash': '33290e505785abd0f6458fcf5a61adb8',
      'identificacao': 'Laudo 2269/2017-SETEC/SR/PF/PR',
      'ocorrencias': 23,
      'ocorrencias_codigo_unidade': {'3547': 23},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 23},
      'quesitos': ['1. Qual a natureza e características do(s) '
                   'material(ais) submetido(s) a exame ?',
                   '2. Quais os números telefônicos de habilitação '
                   'associados?',
                   '3. Quais os registros correspondentes às chamadas '
                   'telefônicas recebidas, efetuadas e não atendidas?',
                   '4. Quais os registros constantes da(s) agenda(s) '
                   'telefônica(s)?',
                   '5. Extrair os dados e metadados relativos às '
                   'comunicações eletrônicas (por exemplo WhatsApp) '
                   'eventualmente armazenadas nos dispositivos '
                   'informáticos, devendo a apresentação destes dados ser '
                   'em formato de texto legível e navegável;',
                   '6. Outros dados julgados úteis.'],
      'tamanho': 584,
      'texto': '1. Qual a natureza e características do(s) material(ais) '
               'submetido(s) a exame ? 2. Quais os números telefônicos de '
               'habilitação associados? 3. Quais os registros '
               'correspondentes às chamadas telefônicas recebidas, efetuadas '
               'e não atendidas? 4. Quais os registros constantes da(s) '
               'agenda(s) telefônica(s)? 5. Extrair os dados e metadados '
               'relativos às comunicações eletrônicas (por exemplo WhatsApp) '
               'eventualmente armazenadas nos dispositivos informáticos, '
               'devendo a apresentação destes dados ser em formato de texto '
               'legível e navegável; 6. Outros dados julgados úteis.'},
     {'codigo_documento_interno': '33210525',
      'hash': '8ad4c5703e3162acdc981ff8b8a62ec0',
      'identificacao': 'Laudo 568/2017-SETEC/SR/PF/PR',
      'ocorrencias': 16,
      'ocorrencias_codigo_unidade': {'3547': 16},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 16},
      'quesitos': ['1. Qual a natureza e características do(s) '
                   'material(ais) submetido(s) a exame?',
                   '2. Quais os números telefônicos de habilitação '
                   'associados?',
                   ...
                   '7. Extrair os dados e metadados relativos às '
                   'comunicações eletrônicas (por exemplo WhatsApp), '
                   'fotos e vídeos eventualmente armazenados nos '
                   'dispositivos informáticos, devendo a apresentação '
                   'destes dados ser em formato de texto legível e '
                   'navegável.',
                   '8. É possível, a partir dos dados constantes no '
      ...
      'texto': '1. Qual a natureza e características do(s) material(ais) '
               ...
               'recuperadas? 7. Extrair os dados e metadados relativos às '
               'comunicações eletrônicas (por exemplo WhatsApp), fotos e '
               'vídeos eventualmente armazenados nos dispositivos '
               'informáticos, devendo a apresentação destes dados ser em '
               'formato de texto legível e navegável. 8. É possível, a '
               'partir dos dados constantes no objeto de exame, identificar '
               'o usuário do aparelho celular? 9. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '32797237',
      'hash': 'ef33db9d6168807981dfc4b25edd77b0',
      'identificacao': 'Laudo 346/2017-SETEC/SR/PF/PR',
      'ocorrencias': 4,
      'ocorrencias_codigo_unidade': {'3547': 4},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 4},
      'quesitos': ['I. Qual a natureza dos equipamentos apresentados para '
                   ...
                   'VIII. Quais as mensagens de texto recebidas '
                   'constantes na memória dos aparelhos?',
                   'IX. Existem chamadas ou mensagem de texto trocadas '
                   'entre os numerais constantes em cada um dos '
                   'aparelhos? (cruzamento de dados)',
                   'X. Outros dados julgados úteis.'],
      'tamanho': 738,
      'texto': 'I. Qual a natureza dos equipamentos apresentados para exame? '
               ...
               'constantes na memória dos aparelhos? VIII. Quais as '
               'mensagens de texto recebidas constantes na memória dos '
               'aparelhos? IX. Existem chamadas ou mensagem de texto '
               'trocadas entre os numerais constantes em cada um dos '
               'aparelhos? (cruzamento de dados) X. Outros dados julgados '
               'úteis.'},
     {'codigo_documento_interno': '36665566',
      'hash': '9d3a9931d7daf210b107e8d58e55d081',
      'identificacao': 'Laudo 2013/2017-SETEC/SR/PF/PR',
      'ocorrencias': 9,
      'ocorrencias_codigo_unidade': {'3547': 9},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 9},
      'quesitos': ['1. Qual a natureza e características do(s) '
      ...
      'texto': '1. Qual a natureza e características do(s) material(ais) '
               ...
               'devendo a apresentação destes dados ser em formato de texto '
               'legível e navegável. 6. Outros dados julgados úteis.'},
     {'codigo_documento_interno': '33138800',
      'hash': '234e08fbd1dec9950d8017782eb67a97',
      'identificacao': 'Laudo 535/2017-SETEC/SR/PF/PR',
      'ocorrencias': 7,
      'ocorrencias_codigo_unidade': {'3547': 7},
      'ocorrencias_sigla_unidade': {'SETEC/SR/PF/PR': 7},
      'quesitos': ['1. Qual a natureza e características do(s) '
                   'material(ais) submetidos(s) a exame?',
                   '2. Quais os números telefônicos de habilitação '
                   'associados?',
                   '3. Há registros correspondentes às chamadas '
                   'telefônicas recebidas, efetuadas e não atendidas? '
      ...
      'tamanho': 891,
      'texto': '1. Qual a natureza e características do(s) material(ais) '
               'submetidos(s) a exame? 2. Quais os números telefônicos de '
               'habilitação associados? 3. Há registros correspondentes às '
               'chamadas telefônicas recebidas, efetuadas e não atendidas? '
               'Caso positivo, relacionar. 4. Há registros constantes da(s) '
               'agenda(s) telefônica(s)? Caso positivo, relacionar. 5. '
    ...
    for quesitacao in quesitacoes:
        print("-"*100)
        print_tela(quesitacao['hash'])
        for q in quesitacao['quesitos']:
            print_tela(q)

    #die('ponto5290')

    # Fazendo teste...com template fixo
    # TODO: Fazer download do template
    caminho_template = "D:/Exames_andamento/documentos_produzidos/template_modelo_sapi_laudo_1_2.odt"
...
def teste_carrega_valida_modelo(caminho_arquivo_modelo_odt, template=False):
    ...
    try:
        ...
        print_tela_log("- ERRO na criação do arquivo de saída '" + caminho_arquivo_modelo_odt + "'")
        print_tela_log("- ERRO: ", e)
        print_tela_log(
            "- Verifique se arquivo não está aberto, ou se existe alguma outra condição que possa estar impedindo a sua criação")
        return None
    ...
    if not substitui_content_xml_em_odt(Godt_raiz, caminho_novo):
        die('ponto6866')
...
def teste_xml_edit():
    ...
    print("==== Namespace ====")
    odt_print_ns(parse_and_get_ns(caminho_xml))

    # Registra os namespace no xml_tree
    ns=parse_and_get_ns(caminho_xml)
    #var_dump(ns)
    #die('ponto6921')
    qtd = 1
    for k in ns:
    ...
    print("============== DEPOIS =====================")
    odt_print_ns(parse_and_get_ns(caminho_xml1))
    # odt_dump(xml_odt)
    die('ponto6917')
...
if __name__ == '__main__':
    ...
    die('ponto6950')
    #caminho_template = "D:/tempr/xml/LAUDO_MODELO_VERSAO_2_3.odt"
    #teste_carrega_valida_modelo(caminho_template)
