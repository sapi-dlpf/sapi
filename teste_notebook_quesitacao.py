...
class TelaQuesitacao():
    ...
    def constroi_frame_principal(self, parent):
        ...
        self.label_tooltip1 = GuiCreateToolTip(label, "Clique para abrir SisCrim no browser")
        ...
        self.text_quesitacao_atual = tk.Text(frame_geral, height=10, width=100)
    ...
    def frame_escolha_quesitacao(self, parent, dic_quesitos):
        ...
        # Quantidade de quesitos
        coluna=10
        ttk.Label(frame_aux, text="Qtd. Quesitos:").grid(column=coluna, row=linha)  # 1
        ...
        label.grid(column=30, row=linha)
        label.configure(foreground='blue')
        GuiCreateToolTip(label, ("Utilize os filtros, principalmente o filtro por quantidade de quesitos.\n"
                                 "Para selecionar quesitação: clique duplo.\n"
                                 "Para mudar ordenação: clique no título da coluna de ordenação."))
        ...
        self.trv.bind('<Button-1>', self.treeview_clique)
    ...
    def popula_tree_view(self):
        ...
        # Seleciona as quesitaçõe que tem alguma linha de quesito selecionada
        ...
        # Armazena e calcula as larguras das colunas
        col_quesitacao_max = 0
        col_quesito_max = 0
        for id_quesitacao in self.dic_quesitos:
            quesitacao=dic_quesitos[id_quesitacao]

            # Despreza se não atender filtro de quantidade de quesitos
            if self.filtro_qtd_quesitos is not None:
                if quesitacao['quantidade_quesitos'] !='Qualquer':
            ...
            # Armazena a largura necessária para acomodar o maior id de quesitação
            col_w = tkFont.Font().measure(id_quesitacao)
            if col_quesitacao_max < col_w:
                col_quesitacao_max = col_w
            ...
            for linha in quesitacao['texto_quesitos'].split('\n'):
                ...
                # Inclui registro
                self.trv.insert('', 'end',
                           values=(id_quesitacao, linha), tags=(tag))
        ...
        self.trv.column('quesitação', width=col_quesitacao_max)
        self.trv.column('quesito', width=col_quesito_max)

        # Cor por tag
        self.trv.tag_configure('nao_selecionado', background='')
        self.trv.tag_configure('selecionado', background='blue')
    ...
    def combo_itemizador_change(self, event):
        self.value_of_combo = self.combo_itemizador.get()
        print("mudou itemizador: ",self.value_of_combo)
...
root.geometry("{0}x{1}+0+0".format(
    root.winfo_screenwidth() - pad, root.winfo_screenheight() - pad - 300))
